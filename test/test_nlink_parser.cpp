...
static const float absError = 0.001f;

#define COMPARE_ARRAY(SRC, DEST, ABS_ERROR)                                    \
  {                                                                            \
    const auto &ACTUAL = SRC;                                                  \
    const auto &EXPECTED = std::vector<float> DEST;                            \
    for (size_t _INDEX = 0; _INDEX < EXPECTED.size(); ++_INDEX) {              \
      EXPECT_NEAR(EXPECTED.at(_INDEX), ACTUAL.at(_INDEX), ABS_ERROR);          \
    }                                                                          \
  }

TEST(NLinkParser, linktrack) {
  NFrameExtraction frameExtraction;
  LinkTrack::Init init(&frameExtraction, nullptr);
  uint8_t data[1024];
  {
    ...
    auto dataLength = stringToHex(string, data);
    frameExtraction.unpackData(data, dataLength);

    auto &msgData = LinkTrack::Init::msgAnchorFrame0Data_;
    COMPARE_ARRAY(msgData.tag[0].pos, ({2.895f, 2.419f, -0.263f}), absError)
    COMPARE_ARRAY(msgData.tag[0].dis, ({3.64f, 3.34f, 4.9f, 4.93f, 0, 0, 0, 0}),
                  absError)
    COMPARE_ARRAY(msgData.tag[1].pos, ({2.435f, 2.399f, -1.117f}), absError)
    COMPARE_ARRAY(msgData.tag[1].dis, ({3.18f, 2.98f, 5.3f, 5.31f, 0, 0, 0, 0}),
                  absError)
    EXPECT_EQ(msgData.localTime, 33313);
    EXPECT_EQ(msgData.systemTime, 32000);
    EXPECT_NEAR(msgData.voltage, 4.995f, absError);
  }
  {
    auto string =
    ...
    auto dataLength = stringToHex(string, data);
    frameExtraction.unpackData(data, dataLength);
    auto &msgData = LinkTrack::Init::msgTagFrame0Data_;
    COMPARE_ARRAY(msgData.pos, ({2.702f, -0.091f, 1}), absError)
    COMPARE_ARRAY(msgData.vel, ({-0.0038f, -0.0006f, 0}), 0.0001f)
    COMPARE_ARRAY(msgData.dis, ({3.125f, 5.539f, 6.861f, 4.684f, 0, 0, 0, 0}),
                  absError)
    COMPARE_ARRAY(msgData.imuGyro, ({0.02767f, 0.008514f, -0.045762f}),
                  0.00001f)
    COMPARE_ARRAY(msgData.imuAcc, ({0.251265f, 9.74908f, -0.040681f}), 0.00001f)
    COMPARE_ARRAY(msgData.angle, ({144.49f, 97.17f, -14.68f}), 0.01f)
    COMPARE_ARRAY(msgData.q, ({-0.656777f, -0.125699f, -0.739489f, 0.0512801f}),
                  0.00001f)
    COMPARE_ARRAY(msgData.eop, ({0.11f, 0.16f, 2.55f}), 0.01f)
    EXPECT_EQ(msgData.localTime, 44556);
    EXPECT_EQ(msgData.systemTime, 71627);
    EXPECT_NEAR(msgData.voltage, 4.948f, absError);
  }
  {
    auto string =
    ...
    auto dataLength = stringToHex(string, data);
    frameExtraction.unpackData(data, dataLength);
    auto &msgData = LinkTrack::Init::msgNodeFrame0Data_;
    EXPECT_EQ(msgData.node[0].data.size(), 9);
    EXPECT_EQ(msgData.node[1].data.size(), 37);
  }
  {
    auto string =
    ...
    auto dataLength = stringToHex(string, data);
    frameExtraction.unpackData(data, dataLength);
    auto &msgData = LinkTrack::Init::msgNodeFrame1Data_;
    COMPARE_ARRAY(msgData.node[0].pos, ({2.911f, 2.438f, -0.101f}), absError)
    COMPARE_ARRAY(msgData.node[1].pos, ({2.451f, 2.373f, -0.828f}), absError)
    EXPECT_NEAR(msgData.node[1].pos[1], 2.373, 0.001);
    EXPECT_EQ(msgData.systemTime, 33000);
    EXPECT_EQ(msgData.localTime, 34304);
    EXPECT_NEAR(msgData.voltage, 4.936f, absError);
  }
  {
    auto string =
    ...
    auto dataLength = stringToHex(string, data);
    frameExtraction.unpackData(data, dataLength);
    auto &msgData = LinkTrack::Init::msgNodeFrame2Data_;
    COMPARE_ARRAY(msgData.pos, ({2.782f, -0.033f, 1}), absError)
    COMPARE_ARRAY(msgData.vel, ({-0.0006f, 0.0026f, 0}), 0.0001f)
    COMPARE_ARRAY(msgData.imuGyro, ({0.02767f, 0.00958f, -0.04576f}), 0.00001f)
    COMPARE_ARRAY(msgData.imuAcc, ({0.224942f, 9.73712f, -0.05504f}), 0.00001f)
    COMPARE_ARRAY(msgData.angle, ({90.69f, 91.93f, -88.48f}), 0.01f)
    COMPARE_ARRAY(msgData.q, ({0.691282f, 0.694677f, 0.154792f, -0.110552f}),
                  0.00001f)
    COMPARE_ARRAY(msgData.eop, ({0.06f, 0.09f, 2.55f}), 0.01f)
    EXPECT_EQ(msgData.localTime, 9157);
    EXPECT_EQ(msgData.systemTime, 1926842);
    EXPECT_NEAR(msgData.voltage, 4.973f, absError);
    EXPECT_NEAR(msgData.node[0].dis, 3.179f, absError);
    EXPECT_NEAR(msgData.node[0].fpRssi, -88.5f, absError);
    EXPECT_NEAR(msgData.node[0].rxRssi, -79.5f, absError);
    EXPECT_NEAR(msgData.node[1].dis, 5.548f, absError);
    EXPECT_NEAR(msgData.node[1].fpRssi, -90, absError);
    EXPECT_NEAR(msgData.node[1].rxRssi, -80.5f, absError);
    EXPECT_NEAR(msgData.node[2].dis, 6.728f, absError);
    EXPECT_NEAR(msgData.node[2].fpRssi, -101, absError);
    EXPECT_NEAR(msgData.node[2].rxRssi, -79.5f, absError);
    EXPECT_NEAR(msgData.node[3].dis, 4.651f, absError);
    EXPECT_NEAR(msgData.node[3].fpRssi, -99, absError);
    EXPECT_NEAR(msgData.node[3].rxRssi, -80, absError);
  }
  {
    auto string =
    ...
    auto dataLength = stringToHex(string, data);
    frameExtraction.unpackData(data, dataLength);
    auto &msgData = LinkTrack::Init::msgNodeFrame3Data_;
    EXPECT_EQ(msgData.localTime, 463352);
    EXPECT_EQ(msgData.systemTime, 7262319);
    EXPECT_NEAR(msgData.voltage, 4.954f, absError);
    EXPECT_NEAR(msgData.node[0].dis, 2.85f, absError);
    EXPECT_NEAR(msgData.node[0].fpRssi, -90.5f, absError);
    EXPECT_NEAR(msgData.node[0].rxRssi, -79.5f, absError);
    EXPECT_NEAR(msgData.node[1].dis, 6.051f, absError);
    EXPECT_NEAR(msgData.node[1].fpRssi, -91, absError);
    EXPECT_NEAR(msgData.node[1].rxRssi, -80, absError);
    EXPECT_NEAR(msgData.node[2].dis, 7.304f, absError);
    EXPECT_NEAR(msgData.node[2].fpRssi, -85, absError);
    EXPECT_NEAR(msgData.node[2].rxRssi, -79.5f, absError);
    EXPECT_NEAR(msgData.node[3].dis, 5.35f, absError);
    EXPECT_NEAR(msgData.node[3].fpRssi, -92, absError);
    EXPECT_NEAR(msgData.node[3].rxRssi, -80, absError);
  }
}
...
TEST(NLinkParser, tofsense) {
  NFrameExtraction frameExtraction;
  TOFSense::Init init(&frameExtraction, nullptr);
  ...
  EXPECT_EQ(ntsFrame0_.data.id, 0);
  EXPECT_EQ(ntsFrame0_.data.systemTime, 17858);
  EXPECT_NEAR(ntsFrame0_.data.distance, 0.64, 0.001);
  EXPECT_EQ(ntsFrame0_.data.distanceStatus, 0);
  EXPECT_EQ(ntsFrame0_.data.signalStrength, 8);
}
